...
{
    GPUImageOutput<GPUImageInput> *filter_imageView;
}
...
- (void)viewDidLoad {
    ...
    filter_imageView = (GPUImageOutput<GPUImageInput> *)filterView;
...
- (void)synchronizePlayerWithEditor
{
    ...
    [filter addTarget:filter_imageView];
    [filter addTarget:self.movieWriter];
    
    
    [self.imageMovieComposition enableSynchronizedEncodingUsingMovieWriter:self.movieWriter];
    //self.imageMovieComposition.audioEncodingTarget = self.movieWriter; //bias 由于我们demo暂时不加入音频录制
